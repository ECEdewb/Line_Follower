...
void __ISR(_TIMER_4_VECTOR, IPL7AUTO) Timer4ISR(void) {
    ...
    if(TimeFlag){ // asssuming that mode 2 is the racing mode
        counterMS++;
        if(counterMS == 10){
           TimeMS++;
           ssdnumMS = TimeMS;
           ...
           SSD_WriteDigits(ssdnumMS,ssdnum1,ssdnum2,ssdnum3,0,1,0,0);  //includes .1 seconds
           counterMS = 0;
...
void updateLCD()
{
    LCD_WriteStringAtPos("The Joe-inator",0,1);
}

void moveRobot(int dir){
    switch(dir){
        case 0:                 // "STP"
            if(Time >= 40){     // "tests to see if the Time on the SSD is > 35 seconds
                servo1PWM(1500);
                servo2PWM(1500);
                state = mode0;  //stops the timer
                TimeFlag = 0;
            }//if Time > 35 seconds
            break;
        case 1:                 // "FWD"
